function Arc(context, args) {
	...
	this.cubicR.rp1 = (args.params && args.params.cubicR) ? args.params.cubicR.rp1 : randomBetween(TreeManager.params.branch.cubicR.min, TreeManager.params.branch.cubicR.max)/100*this.maths.lengthSide;
	this.cubicR.rp2 = (args.params && args.params.cubicR) ? args.params.cubicR.rp2 : randomBetween(TreeManager.params.branch.cubicR.min, TreeManager.params.branch.cubicR.max)/100*this.maths.lengthSide;
...
Arc.prototype.calcShapeCoord = function(){
	...
	var xp1 = randomBetween(-1*limxp1, limxp1);  //X aléatoire dans l'intervale des limite définis précédemment
	var yp1 = (xp1 ===0) ? this.cubicR.rp1 : Math.sqrt(Math.pow(this.cubicR.rp1, 2) - Math.pow(Math.abs(xp1), 2)); 
	
	//Idem second point de controle
	var xp2 = randomBetween(-1*limxp2, limxp2); 
